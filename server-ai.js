...
function imageDimensions(file){return createImageBitmap(file,{imageOrientation:'from-image'}).then(b=>{const x={w:b.width,h:b.height};b.close?.();return x;});}
...
async function preprocess(file){
  const bm=await createImageBitmap(file,{imageOrientation:'from-image'});const sw=bm.width,sh=bm.height;let max=MAX_DIM,q=.82,blob,canvas;
  for(let attempt=0;attempt<6;attempt++){
    ...
    if(blob.size<=MAX_BYTES)break;canvas.width=canvas.height=1;if(q>.62)q-=.08;else max=Math.max(760,Math.round(max*.84));
  }
  bm.close?.();
...
async function cropOriginal(file,bbox){
  const bm=await createImageBitmap(file,{imageOrientation:'from-image'});const [x,y,w,h]=bbox||[0,0,1,1],pad=.035;const x0=Math.max(0,x-pad),y0=Math.max(0,y-pad),x1=Math.min(1,x+w+pad),y1=Math.min(1,y+h+pad);const sx=Math.round(x0*bm.width),sy=Math.round(y0*bm.height),sw=Math.max(1,Math.round((x1-x0)*bm.width)),sh=Math.max(1,Math.round((y1-y0)*bm.height));const scale=Math.min(1,1000/Math.max(sw,sh));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(sw*scale));c.height=Math.max(1,Math.round(sh*scale));c.getContext('2d').drawImage(bm,sx,sy,sw,sh,0,0,c.width,c.height);bm.close?.();const blob=await new Promise((res,rej)=>c.toBlob(b=>b?res(b):rej(new Error('crop failed')),'image/webp',.9));c.width=c.height=1;return URL.createObjectURL(blob);
}
...
async function analyzeOne(file,sourceIndex,mode,onProgress){
  const prep=await preprocess(file);onProgress?.({type:'route',sourceIndex,mode,sourceWidth:prep.sourceWidth,sourceHeight:prep.sourceHeight,processedWidth:prep.width,processedHeight:prep.height});
  ...
  const items=[];for(let i=0;i<(body.items||[]).length;i++){const it=body.items[i],meta=CAT_MAP[it.category]||{name:it.label||'單品',cat:'未分類'};let photo=it.cutout_url||it.mask_url||null,photoType=photo?'mask':'bbox-crop';if(!photo&&it.bbox)photo=await cropOriginal(file,it.bbox);if(!photo&&it.visualization_url){photo=it.visualization_url;photoType='visualization'};if(!photo)continue;const avg=await sampleColor(file,it.bbox||[0,0,1,1]);const bbox=it.bbox||[0,0,1,1];items.push({id:`${sourceIndex}-srv-${i}-${crypto.randomUUID?.()||Math.random().toString(36).slice(2)}`,name:meta.name,cat:meta.cat,label:it.label||it.category,sourceIndex,sourceName:file.name,sourceWidth:prep.sourceWidth,sourceHeight:prep.sourceHeight,processedWidth:prep.width,processedHeight:prep.height,routeMode:mode==='accessory'?'accessory':mode,classificationConfidence:Math.round((it.confidence||0)*100),needsGenericCategoryReview:(it.confidence||0)<.45||meta.cat==='未分類',reviewReason:'Server AI 信心偏低，請確認類別',photo,photoType,bbox,areaRatio:Math.max(.001,bbox[2]*bbox[3]),aspect:bbox[2]/Math.max(.001,bbox[3]),avgColor:avg,serverDebug:body.debug||null});}
  return items;
}
window.AIWardrobeSegmentation={version:'4.3-server',modelId:'vufinder/sam3',async health(){const r=await fetch('/api/analyze');return r.json()},async segmentFiles(files,onProgress,options={}){const all=[];const mode=options.mode||'auto';for(let i=0;i<files.length;i++){onProgress?.({type:'file',index:i,total:files.length,name:files[i].name});onProgress?.({type:'inference',sourceIndex:i,stage:'start'});const one=await analyzeOne(files[i],i,mode,onProgress);all.push(...one);onProgress?.({type:'inference',sourceIndex:i,stage:'done',count:one.length,mode});await yieldUI()}return{items:all,duplicates:findDuplicates(all)}}};
