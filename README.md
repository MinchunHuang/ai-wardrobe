# AI Wardrobe V5.3.2 — Local AI Stability + Mask Fix

V5.3.2 is a corrective benchmark release based on real iPhone tests.
...
1. **Outfit parser mask bug**
   ...
   - V5.3.2 builds alpha from mask pixel values instead.
...
2. **U2Netp cutout was too aggressive**
   ...
   - V5.3.2 follows soft min-max alpha and adds a completeness gate.
...
3. **Classification honesty**
   ...
   - V5.3.2 no longer presents a medium-confidence label as certain.
...
This is still a benchmark stage. The purpose of V5.3.2 is to separate implementation bugs from actual model limitations before replacing another model.
...
## V5.3.2 cold-start fix
- iOS 人物穿搭 / Auto：先載入 SegFormer，再解碼使用者照片，避免第一次下載模型與 24MP 影像工作副本的記憶體尖峰重疊。
- 人物穿搭 iOS 工作副本降為最長邊 1200px；單件/平拍仍保留 1600px。
- Scene router 工作圖降為 720px。
- UI 顯示『首次準備人物穿搭模型』與『模型已就緒』階段。
