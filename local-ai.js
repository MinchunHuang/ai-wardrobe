...
async function createOrtSession(url,onProgress,label){
  ...
  if(navigator.gpu){try{const s=await ort.InferenceSession.create(buf,{executionProviders:['webgpu'],graphOptimizationLevel:'all'});runtimeInfo.backend='WebGPU';return s;}catch(e){console.warn(label,'WebGPU fallback',e);}}
  const s=await ort.InferenceSession.create(buf,{executionProviders:['wasm'],graphOptimizationLevel:'all'});runtimeInfo.backend='WASM';return s;
}
...
async function loadBitmap(file){if('createImageBitmap'in window)return createImageBitmap(file,{imageOrientation:'from-image'});return new Promise((resolve,reject)=>{const u=URL.createObjectURL(file),img=new Image();img.onload=async()=>{try{resolve(await createImageBitmap(img));}catch(e){reject(e)}finally{URL.revokeObjectURL(u)}};img.onerror=reject;img.src=u;});}
...
async function makeSemanticCutout(bm,segments,labels){
  ...
  const sx=bm.width/mw,sy=bm.height/mh,x0=mx0*sx,y0=my0*sy,cw=mcw*sx,ch=mch*sy,scale=Math.min(1,1200/Math.max(cw,ch)),ow=Math.max(1,Math.round(cw*scale)),oh=Math.max(1,Math.round(ch*scale));
...
async function sceneHasHuman(file,onProgress){
  const bm=await loadBitmap(file),url=await bitmapToBlobURL(bm,900),pipe=await getOutfitPipeline(onProgress),segments=await pipe(url);URL.revokeObjectURL(url);bm.close?.();
